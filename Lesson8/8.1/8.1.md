...
## Step 1 @unplugged
In Python, collections are data structures that allow you to store and organize multiple values or elements. Collections may also be called **lists**. Python provides several built-in collection types, such as arrays, sets, and maps. We are going to start by learning about arrays. 


**Arrays:** Arrays are the most commonly used collection (or list) type in Python. They are ordered, indexed collections of values, where each value is associated with a numeric index. Arrays can store values of different types, including numbers, strings, objects, and even other arrays. You can think of an array as a list.  Here's an example of an array:
...
Let's first learn how to create an array in MakeCode Python.  
