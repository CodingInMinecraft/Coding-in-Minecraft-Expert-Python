# Expert Coding using Python - Lesson 1 Activity 9
### @explicitHints true
...
To add an argument the on chat command, type the variable name in the parentheses on the first line.  We are going to use a variable called Y.  Type Y in the parentheses. 
#### ~ tutorialhint
...
Next, we want to add to the code what we want to happen when the code runs.  We want the player to jump to the Y position that the argument will set.  First, add the player teleport code into the chat command.  Then change the Y coordinate to the variable Y. 
