# Expert Coding using Python - Lesson 1 Activity 1
### @explicitHints true
...
*In Python, variable naming styles can vary depending on the version or platform you're using. 
Camel Case (like myVariableName) is accepted in MakeCode's version of Python, which is what we'll be using in this course. 
That means you're welcome to use Camel Case when naming your variables. 
However, in standard Python, the preferred style is Snake Case, where all letters are lowercase and words are separated by underscores (like my_variable_name). 
Snake Case improves readability and follows Python's official style guidelines. 
While we'll primarily use Camel Case in this course, Snake Case will also work—so feel free to use whichever style you're most comfortable with!
