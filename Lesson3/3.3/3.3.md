# Expert Coding using Python - Lesson 3 Activity 6
### @explicitHints true
...
Next add the code that if the agent does inspect a block, it will destroy forward and move forward by 1.  If it does not inspect a block, it will just move forward by 1. 
...
3.  You will know your code is correct when the agent destroys the block in front of it and moves forward by 1. 
...
We need to clear a 5 x 5 area which means we need to run this if / else statement 5 times to clear one row, if there were 5 blocks in that row. However, because the first row only has 4 blocks, we only need to run it 4 times this row.  Copy and paste the code **inside the chat command** to repeat it 4 times. 
...
2. Use your agent device to reset the area. 
3. Open the chat and type clear. 
4.  You will know your code is correct when the agent destroys the 3 rows of blocks. 
