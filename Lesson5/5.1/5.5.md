...
Next, inside the chat command, we want the agent to continue to plant as long as it does not detect a block in front of it.  

We will use a while loop but include the boolean not so that the loop would continue while there is NO block in front of the agent. 
The loop will stop as soon as the block is detected. 
