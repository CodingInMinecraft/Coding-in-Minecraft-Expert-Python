...
@blocks.on_block_placed(HAY_BLOCK)
def on_hay_placed():
    global hay
    hay += 1
    if hay == 4:
...
@blocks.on_block_placed(BEE_NEST)
def on_bee_nest_placed():
    global hives
    hives += 1
    player.say(str(hives))
    if hives >= 3:
        for _ in range(4):
